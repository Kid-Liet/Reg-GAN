...
class ResnetGenerator2(nn.Module):
    def __init__(self, input_nc,output_nc, ngf=64, n_blocks=7, img_size=256, light=False):
        ...
        for i in range(1, 2):   #1+3*2^0 + 3*2^1 =10        
            ...
            model += [nn.ReplicationPad2d(1),
                      nn.utils.spectral_norm(
                      nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=4, stride=2, padding=0, bias=True)),
        ...
        UpBlock0 = [nn.ReplicationPad2d(1),
        ...
        if self.light:
            ...
            FC = [nn.Linear(img_size * ngf * ngf//4, ngf * mult, bias=False),
                  nn.ReLU(True),
                  nn.Linear(ngf * mult, ngf * mult, bias=False),
                  nn.ReLU(True)]
        ...
        UpBlock2 += [nn.ReplicationPad2d(3),
                     nn.Conv2d(int(ngf/2), output_nc, kernel_size=7, stride=1, padding=0, bias=False),
                     nn.Tanh()]
...
class Discriminator2(nn.Module):
    def __init__(self, in_channel=1, out_channel=1,ndf=32,n_layers=7):
        ...
        mult = 2 ** (n_layers - 4 - 1)
        Dis0_1 = [nn.ReplicationPad2d(1),     #1+3*2^0 + 3*2^1 + 3*2^2 +3*2^3 = 46
                nn.utils.spectral_norm(
                nn.Conv2d(ndf * mult, ndf * mult * 2, kernel_size=4, stride=1, padding=0, bias=True)),
                nn.LeakyReLU(0.2, True)]
        mult = 2 ** (n_layers - 4)
        ...
        for i in range(n_layers - 4, n_layers - 2):   # 1+3*2^0 + 3*2^1 + 3*2^2 + 3*2^3=46, 1+3*2^0 + 3*2^1 + 3*2^2 +3*2^3 +3*2^4 = 94
            mult = 2 ** (i - 1)
            Dis1_0 += [nn.ReplicationPad2d(1),
                      nn.utils.spectral_norm(
                      nn.Conv2d(ndf * mult, ndf * mult * 2, kernel_size=4, stride=2, padding=0, bias=True)),
                      nn.LeakyReLU(0.2, True)]

        mult = 2 ** (n_layers - 2 - 1)
        Dis1_1 = [nn.ReplicationPad2d(1),  #1+3*2^0 + 3*2^1 + 3*2^2 +3*2^3 +3*2^4 + 3*2^5= 94 + 96 = 190
                nn.utils.spectral_norm(
                nn.Conv2d(ndf * mult, ndf * mult * 2, kernel_size=4, stride=1, padding=0, bias=True)),
                nn.LeakyReLU(0.2, True)]
        mult = 2 ** (n_layers - 2)
