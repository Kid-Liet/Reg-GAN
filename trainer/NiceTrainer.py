...
class Nice_Trainer():
    def __init__(self, config):
        ...
        transforms_1 = [ToPILImage(),
                   RandomAffine(degrees=1,translate=[0.01, 0.01],scale=[0.98, 1.02],fillcolor=-1),
                   ToTensor(),
                   Resize(size_tuple = (config['size'], config['size']))]
    
#         transforms_2 = [ToPILImage(),
#                    RandomAffine(degrees=5,translate=[0.05, 0.05],scale=[0.9, 1.1],fillcolor=-1),
#                    ToTensor(),
#                    Resize(size_tuple = (config['size'], config['size']))]

        self.dataloader = DataLoader(ImageDataset(config['dataroot'], transforms_1=transforms_1, transforms_2=transforms_1, unaligned=False),
            ...
            for i, batch in enumerate(self.dataloader):
                ...
                if self.config['bidirect']:   # b dir
                    if self.config['regist']:    # + reg 
                        ...
                        D_loss_A = (D_ad_loss_GA + D_ad_cam_loss_A + D_ad_loss_LA)
                        D_loss_B = (D_ad_loss_GB + D_ad_cam_loss_B + D_ad_loss_LB)
                        ...
                        G_loss_A = (G_ad_loss_GA + G_ad_cam_loss_A + G_ad_loss_LA ) + 10 * G_cycle_loss_A + 10 * G_ident_loss_A 
                        G_loss_B = (G_ad_loss_GB + G_ad_cam_loss_B + G_ad_loss_LB ) + 10 * G_cycle_loss_B + 10 * G_ident_loss_B 
                    ...
                    if self.config['regist']:    # + reg 
                        ...
                        adv_loss = self.MSE_loss(fake_GB_logit, torch.ones_like(fake_GB_logit).cuda())+                                                        self.MSE_loss(fake_LB_logit, torch.ones_like(fake_LB_logit).cuda())+                                                        self.MSE_loss(fake_B_cam_logit, torch.ones_like(fake_B_cam_logit).cuda())
                        ####smooth loss
                        SM_loss = 10*smooothing_loss(Trans)
                ...
                self.logger.log({'loss_D_B': Discriminator_loss,},
